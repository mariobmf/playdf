    ...
    if (pdfFileCache && playbackFileCache) {
      ...
      if (mediaLibraryPermissions.granted) {
        if (FileSystem.documentDirectory) {
          const directoryPlayback = `${FileSystem.documentDirectory}albums/${playbackName}`; // Diretorio do Playback
          ...
          if (!directoryPlaybackInfo.exists) {
            await FileSystem.makeDirectoryAsync(directoryPlayback, {
              intermediates: true,
            }); // Crio o diretorio
            ...
            const name = `playback${playbackFileCache.name.slice(
              playbackFileCache.name.lastIndexOf('.'),
            )}`;
            await StorageAccessFramework.copyAsync({
              from: playbackFileCache.uri,
              to: `${directoryPlayback}/${name}`,
            });

            addPlaybackUri(directoryPlayback);
